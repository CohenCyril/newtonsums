...
(* [fset x : K in A | P & Q ] :=  [fset x : K in A | P && Q].                *)
(* [fset x in A | P]     :=   [fset x : _ in A | P].                         *)
(* [fset x in A | P & Q ] :=  [fset x : _ in A | P & Q].                     *)
...
Proof. by move=> T0; split; rewrite T0; exists nil. Qed.
...
Coercion finset_of_fset (K : choiceType) (A : finSet K) : {set A} :=
   [set x | x \in {: A}].
